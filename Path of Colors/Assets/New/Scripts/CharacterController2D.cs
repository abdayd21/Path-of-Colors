    ...
    {
        {
            ...
            if (FacingRight)
            {
                transform.localScale = new Vector3(1, 1, 1); // Sa� tarafa bakmak i�in
            }
            else
            {
                transform.localScale = new Vector3(-1, 1, 1); // Sol tarafa bakmak i�in
            }
        }
        UpdateTimers();
        UpdateDash();
        ...
        if (CanMove() && !Dashing && airStaggerTime <= 0)
        {
            if (OnLadder)
            {
                if (direction != 0)
                {
                    ...
                    // SpriteRenderer kullanmadan y�n de�i�tirme i�lemi yap�lacak
                }
                return;
            }

            // Merdiven kontrol� d���nda y�n g�ncelleme
            if (direction != 0)
            {
                FacingRight = direction > 0;
                // SpriteRenderer kullanmadan y�n de�i�tirme i�lemi yap�lacak
